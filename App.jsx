...
async function fetchCountryBorder(name) {
  try {
    const url = `https://nominatim.openstreetmap.org/search?format=json&polygon_geojson=1&limit=1&q=${encodeURIComponent(name)}`;
    const res = await fetch(url);
    const results = await res.json();
    const ring = results && results[0] ? extractMainRing(results[0].geojson) : null;
    if (ring) return decimateRing(ring).map(([lon, lat]) => ({ lat, lon }));
  } catch (e) {
