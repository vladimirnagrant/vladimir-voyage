    ...
    <div style={styles.app}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @keyframes heroPhotoIn { from { opacity: 0; transform: scale(1.04); } to { opacity: 0.85; transform: scale(1); } }
        @keyframes heroNameIn { from { opacity: 0; transform: translateY(22px); } to { opacity: 1; transform: translateY(0); } }
        .hero-photo-in { animation: heroPhotoIn 2.2s ease-out both; }
        .hero-name-in { opacity: 0; animation: heroNameIn 1.1s ease-out 0.5s both; }
        .hero-name-in-delay { animation-delay: 0.75s; }
        .hero-phrase-delay { animation-delay: 1.05s; }
        .hero-cta-delay { animation-delay: 1.35s; }
  ...
  return (
    <header style={styles.hero}>
      ...
      <div style={styles.heroInner}>
        ...
        <button className="cta-bob hero-name-in hero-cta-delay" style={styles.ctaBtn} onClick={onStart}>
...
function extractAllRings(geojson) {
  ...
  if (geojson.type === "MultiPolygon") {
    // outer ring of every landmass (garde toutes les îles, pas seulement la plus grande)
    return geojson.coordinates.map((poly) => poly[0]).filter(Boolean);
  }
  return [];
}
...
async function fetchCountryBorder(name) {
  try {
    ...
    const res = await fetch(url);
    const results = await res.json();
    const rings = results && results[0] ? extractAllRings(results[0].geojson) : [];
    return rings
