...
function playIntroSting() {
  try {
    ...
    // Bus de sortie avec réverbe pour l'ampleur "cinéma"
    const dry = ctx.createGain(); dry.gain.value = 0.95;
    const wet = ctx.createGain(); wet.gain.value = 0.4;
    const reverb = createSoftReverb(ctx, 2.6, 2.6);
    const bus = ctx.createGain();
    bus.connect(dry).connect(ctx.destination);
    bus.connect(wet).connect(reverb).connect(ctx.destination);

    // Un "thump" grave et sourd — le "Ta"
    const thump = (start) => {
      const osc = ctx.createOscillator();
      osc.type = "sine";
      osc.frequency.setValueAtTime(150, start);
      osc.frequency.exponentialRampToValueAtTime(55, start + 0.35);
      ...
      // grain de texture pour donner du corps à l'impact
    ...
    // Un accord métallique/cloche, inharmonique — le "DUM"
    const bellHit = (start) => {
      const base = 220;
      const partials = [1, 2.01, 2.74, 3.53, 4.24];
      partials.forEach((ratio, i) => {
        ...
        gain.gain.exponentialRampToValueAtTime(0.0005, start + 2.2 - i * 0.15);
        osc.connect(gain).connect(bus);
        osc.start(start);
        osc.stop(start + 2.3);
      });
      // renfort grave pour donner du poids sous la cloche
      const sub = ctx.createOscillator();
      sub.type = "sine";
      sub.frequency.value = 82.4;
      const subGain = ctx.createGain();
      subGain.gain.setValueAtTime(0, start);
      subGain.gain.linearRampToValueAtTime(0.35, start + 0.015);
      subGain.gain.exponentialRampToValueAtTime(0.001, start + 1.4);
