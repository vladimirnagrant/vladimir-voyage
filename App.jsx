    ...
    <div style={styles.app}>
      ...
      {showBulkUpload && activeCountry && (
        <BulkUploadModal
          ...
          onDone={(newPages) => { addBulkPages(activeCountry.id, newPages); setShowBulkUpload(false); }}
        />
      )}
  ...
  return (
    <Modal onClose={onClose}>
      ...
      {!uploading && failedCount > 0 && (
        <p style={{ fontSize: 12, color: "#E8785A", marginTop: 8 }}>
          {failedCount} photo{failedCount > 1 ? "s n'ont" : " n'a"} pas pu être envoyée{failedCount > 1 ? "s" : ""} (trop volumineuse ou erreur réseau).
        </p>
      )}
...
function PhotoFormModal({ title, initial, pages, onClose, onSubmit }) {
  ...
  const onPickFile = (e) => {
    ...
    if (!file) return;
    if (file.size > 30 * 1024 * 1024) {
      setUploadError("Cette photo est très volumineuse (plus de 30 Mo) et risque de ne pas se charger. Essaie une autre photo ou réduis sa taille avant de l'envoyer.");
      e.target.value = "";
      return;
    }
    setUploadError("");
