...
function CountryMap({ cities, border }) {
  ...
  const labelRadius = W / 2 - 16;

  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={styles.countryMapSvg}>
      {borderD && <path d={borderD} fill="#C9A87622" stroke="#F4EFE7" strokeWidth="1.2" strokeLinejoin="round" />}
      {points.map((p) => {
        ...
        const anchor = ux > 0.15 ? "start" : ux < -0.15 ? "end" : "middle";
        return (
          <g key={p.id}>
