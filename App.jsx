...
/* ---------------- INDEX SECTION ---------------- */
const IndexSection = React.forwardRef(({ countries, onSelect }, ref) => {
  return (
    <section ref={ref} style={styles.indexSection}>
      ...
      <ul style={styles.indexList}>
        {countries.map((c) => (
          <li key={c.id} style={styles.indexItem}>
            <button style={styles.indexBtn} onClick={() => onSelect(c.id)}>
              <span>{c.name}</span>
...
function PhotoFormModal({ title, initial, pages, onClose, onSubmit }) {
  ...
  const onPickFile = async (e) => {
    ...
    setUploadError("");
    try {
      const path = `${uid("img")}-${file.name}`.replace(/\s+/g, "_");
      ...
      setUploadError("Échec de l'envoi. Vérifie que le bucket 'photos' existe et est public.");
    } finally {
      setUploading(false);
    }
  };
...
function ProfileFormModal({ profile, onClose, onSubmit }) {
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <label style={styles.label}>Photo de fond (noir et blanc conseillé)</label>
      <input style={styles.input} value={photoUrl} onChange={(e) => setPhotoUrl(e.target.value)} placeholder="https://..." />
      <button style={styles.primaryBtn} onClick={() => onSubmit({ prenom, nom, phrase, photoUrl })}>Enregistrer</button>
    </Modal>
  );
}
