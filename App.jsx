...
export default function TravelJournal() {
  ...
  const addCountry = (country) => {
    setData((d) => ({ ...d, countries: [...d.countries, { ...country, id: uid("c"), pages: [] }] }));
  };
...
function Journal({
  ...
  onTouchStart, onTouchEnd, zoomedPhoto, setZoomedPhoto, isAdmin, onEditPhoto, onDeletePhoto, onRequestAddPhoto,
}) {
  if (pages.length === 0) {
    return (
      <div style={styles.journalOverlay}>
        <JournalTopBar country={country} onBackIndex={onBackIndex} pageIndex={0} total={0} />
  ...
  const isSingle = currentPage.photos.length === 1;

  return (
    <div style={styles.journalOverlay} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      <JournalTopBar country={country} onBackIndex={onBackIndex} pageIndex={pageIndex} total={pages.length} />
      ...
      <button style={{ ...styles.pageArrow, left: 4 }} onClick={onPrev} disabled={pageIndex === 0} aria-label="Page précédente">‹</button>
...
function JournalTopBar({ country, onBackIndex, pageIndex, total }) {
  return (
    <div style={styles.journalTopBar}>
      ...
      <span style={styles.journalCounter}>
        {country.name.toUpperCase()} · {total ? `${pageIndex + 1}/${total}` : "0"}
      </span>
    </div>
  );
}
