...
export default function TravelJournal() {
  ...
  const addCountry = (country) => {
    setData((d) => ({ ...d, countries: [...d.countries, { ...country, id: uid("c"), story: "", cities: [], pages: [] }] }));
  };
...
function CoverPage({ country, isAdmin, onEdit }) {
  ...
  return (
    <div style={styles.coverWrap}>
      ...
      {hasCities && <CountryMap cities={country.cities} />}
...
function CountryMap({ cities }) {
  const W = 280, H = 280, PAD = 26;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={styles.countryMapSvg}>
      <path d={pathD} fill="none" stroke="#C9A87666" strokeWidth="1" strokeDasharray="3 5" />
      {points.map((p) => (
        <g key={p.id}>
...
function CoverEditModal({ country, onClose, onSubmit }) {
  ...
  const citiesToText = (cities) => (cities || []).map((c) => `${c.name}, ${c.lat}, ${c.lon}`).join("\n");
  const [citiesText, setCitiesText] = useState(citiesToText(country.cities));

  const parseCities = (text) =>
    text
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean)
      .map((line) => {
        ...
        return { id: uid("vi"), name: parts[0] || "Ville", lat: parseFloat(parts[1]) || 0, lon: parseFloat(parts[2]) || 0 };
      });
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <label style={styles.label}>Villes visitées (une par ligne : Nom, latitude, longitude)</label>
      <textarea style={{ ...styles.input, minHeight: 140, resize: "vertical", fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}
        value={citiesText} onChange={(e) => setCitiesText(e.target.value)}
        placeholder={"Buenos Aires, -34.61, -58.38\nSalta, -24.78, -65.41"} />
      <p style={{ fontSize: 11, color: "#9AA39C", marginTop: 4 }}>
        Astuce : cherche "[ville] coordonnées GPS" pour trouver latitude/longitude.
      </p>
      <button style={styles.primaryBtn} onClick={() => onSubmit({ story, cities: parseCities(citiesText) })}>
        Enregistrer
      </button>
    </Modal>
  );
