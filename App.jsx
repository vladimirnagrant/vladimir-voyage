...
function Globe({ countries, onSelect }) {
  ...
  // Charge les frontières en deux temps : d'abord une version légère (rapide),
  // puis une version détaillée en arrière-plan (petits pays insulaires inclus)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        // Étape 1 : carte légère, affichage quasi immédiat
        const light = await fetch("https://unpkg.com/world-atlas@2.0.2/countries-110m.json").then((r) => r.json());
        if (cancelled) return;
        setFeatures(topojson.feature(light, light.objects.countries).features);

        // Étape 2 : carte détaillée, remplace la légère sans bloquer l'affichage
        const detailed = await fetch("https://unpkg.com/world-atlas@2.0.2/countries-50m.json").then((r) => r.json());
        if (cancelled) return;
        setFeatures(topojson.feature(detailed, detailed.objects.countries).features);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const visited = (feat) => countriesRef.current.some((c) => matchesCountry(feat.properties.name, c.name));
    const findCountry = (feat) => countriesRef.current.find((c) => matchesCountry(feat.properties.name, c.name));

    globe
      .polygonsData([])
      .polygonsTransitionDuration(0)
      .polygonCapColor((feat) => (visited(feat) ? "#C9A876cc" : "#ffffff10"))
      .polygonSideColor(() => "#00000000")
      .polygonStrokeColor((feat) => (visited(feat) ? "#F4EFE7" : "#ffffff22"))
      .polygonAltitude((feat) => (visited(feat) ? 0.012 : 0.006))
      .onPolygonClick((feat) => {
        const c = findCountry(feat);
        if (c) onSelectRef.current(c.id);
      })
      .polygonLabel((feat) => (visited(feat) ? `<div style="color:#F4EFE7;font-family:sans-serif;font-size:13px;font-weight:600;text-shadow:0 1px 3px #000">${feat.properties.name}</div>` : ""));
  ...
  // Applique les frontières + recolore quand pays ou données changent
  useEffect(() => {
    if (!globeRef.current || features.length === 0) return;
    // Pré-calcul : on marque chaque pays comme visité une seule fois (au lieu de
    // refaire la recherche à chaque image de rendu, ce qui ralentissait le globe)
    for (const feat of features) {
      feat.__visited = countries.some((c) => matchesCountry(feat.properties.name, c.name));
    }
    globeRef.current
      .polygonsData(features)
      .polygonCapColor((feat) => (feat.__visited ? "#C9A876cc" : "#ffffff10"))
      .polygonStrokeColor((feat) => (feat.__visited ? "#F4EFE7" : "#ffffff22"))
      .polygonAltitude((feat) => (feat.__visited ? 0.012 : 0.006));
  }, [features, countries]);
...
const styles = {
  ...
  heroPhrase: { fontFamily: "'Work Sans', sans-serif", fontWeight: 700, fontSize: 23, letterSpacing: 1, textTransform: "uppercase", color: "#FFFFFFd9", marginTop: 24, maxWidth: 400, marginLeft: "auto", marginRight: "auto" },
  ctaBtn: {
