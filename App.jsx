...
export default function TravelJournal() {
  ...
  const addCountry = (country) => {
    setData((d) => ({ ...d, countries: [...d.countries, { ...country, id: uid("c"), story: "", cities: [], border: [], pages: [] }] }));
  };
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (prefers-reduced-motion: reduce) {
          .globe-line, .cta-bob { animation: none !important; }
        }
      `}</style>
  ...
  return (
    <header style={styles.hero}>
      <div style={styles.heroPhotoWrap}>
        <img src={profile.photoUrl} alt="" style={styles.heroPhoto} draggable={false} />
        <div style={styles.heroOverlay} />
      </div>
      <div style={styles.heroInner}>
        <h1 style={styles.heroName}>
          <span style={styles.heroFirst}>{profile.prenom}</span>
          <span style={styles.heroLast}>{profile.nom}</span>
        </h1>
        <p style={styles.heroPhrase}>{profile.phrase}</p>
        <button className="cta-bob" style={styles.ctaBtn} onClick={onStart}>
...
function Globe({ countries, onSelect }) {
  ...
  const lonLines = [-60, -30, 0, 30, 60, 90, -90, 120, -120, 150, -150, 180];

  return (
    <svg viewBox="0 0 220 220" style={styles.globeSvg}>
      <circle cx="110" cy="110" r={R} fill="none" stroke="#C9A87655" strokeWidth="1" />
      {latLines.map((lat) => {
        const ry = R * Math.cos((lat * Math.PI) / 180);
        const cy = 110 - R * Math.sin((lat * Math.PI) / 180) * 0; // flat lat rings
        return (
          <ellipse key={lat} className="globe-line" cx="110" cy={110 - (lat / 90) * R * 0}
            rx={R} ry={Math.max(2, R * Math.abs(Math.cos((lat * Math.PI) / 180)))}
            transform={`translate(0 ${(-lat / 90) * 0})`}
            fill="none" stroke="#C9A87633" strokeWidth="0.7"
            strokeDasharray="2 4" style={{ animation: "dashspin 26s linear infinite" }}
          />
        );
      })}
      {lonLines.map((lon, i) => (
        <ellipse key={lon} className="globe-line" cx="110" cy="110" rx={Math.max(3, R * Math.abs(Math.cos((lon * Math.PI) / 180)))}
          ry={R} fill="none" stroke="#C9A87633" strokeWidth="0.7"
          strokeDasharray="2 4" style={{ animation: `dashspin ${20 + i}s linear infinite` }}
        />
      ))}
      <circle cx="110" cy="110" r={R} fill="none" stroke="#C9A876" strokeWidth="1.2" />
      {countries.map((c) => {
        const { x, y, front } = globeProject(c.lat, c.lon);
        const cx = 110 + x * R;
        const cy = 110 - y * R;
        return (
          <g key={c.id} onClick={() => onSelect(c.id)} style={{ cursor: "pointer" }}>
            <circle cx={cx} cy={cy} r="7" fill="transparent" />
            <circle cx={cx} cy={cy} r={front ? 3.4 : 2.2} fill={front ? "#C9A876" : "#C9A87688"} />
          </g>
        );
      })}
    </svg>
...
/* ---------------- INDEX SECTION ---------------- */
const IndexSection = React.forwardRef(({ countries, onSelect, isAdmin, onDelete }, ref) => {
  return (
    <section ref={ref} style={styles.indexSection}>
      ...
      <ul style={styles.indexList}>
        {countries.map((c) => (
          <li key={c.id} style={{ ...styles.indexItem, display: "flex", alignItems: "center" }}>
            <button style={{ ...styles.indexBtn, flex: 1 }} onClick={() => onSelect(c.id)}>
              <span>{c.name}</span>
              <span style={styles.indexCount}>
                {c.pages.reduce((n, pg) => n + pg.photos.length, 0)}
              </span>
            </button>
            {isAdmin && (
              <button
                style={styles.indexDeleteBtn}
...
function Journal({
  ...
  onTouchStart, onTouchEnd, zoomedPhoto, setZoomedPhoto, isAdmin, onEditPhoto, onDeletePhoto, onRequestAddPhoto, onEditCover,
}) {
  ...
  return (
    <div style={styles.journalOverlay} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      ...
      <div style={styles.journalPage}>
        {isCover ? (
          ...
          <div style={{ textAlign: "center" }}>
            ...
            {isAdmin && <button style={styles.addPhotoBtn} onClick={onRequestAddPhoto}>+ Ajouter une photo</button>}
          </div>
      ...
      <button style={{ ...styles.pageArrow, right: 4 }} onClick={onNext} disabled={pageIndex === pages.length - 1} aria-label="Page suivante">›</button>
...
function CountryMap({ cities, border }) {
  ...
  const hasBorder = border && border.length > 0;

  // La frontière (si dispo) définit le cadrage ; sinon on cadre sur les villes seules
  const latSource = hasBorder ? border.map((p) => p.lat) : cities.map((c) => c.lat);
  const lonSource = hasBorder ? border.map((p) => p.lon) : cities.map((c) => c.lon);
  ...
  const borderD = hasBorder
    ? border.map((p, i) => { const pt = project(p.lat, p.lon); return `${i === 0 ? "M" : "L"}${pt.x},${pt.y}`; }).join(" ") + " Z"
    : null;

  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={styles.countryMapSvg}>
      {borderD && <path d={borderD} fill="none" stroke="#F4EFE7" strokeWidth="1.2" strokeLinejoin="round" />}
      {points.map((p) => (
        <g key={p.id}>
          <circle cx={p.x} cy={p.y} r="4" fill="#C9A876" />
          <circle cx={p.x} cy={p.y} r="8" fill="none" stroke="#C9A87655" strokeWidth="1" />
          <text x={p.x} y={p.y - 10} textAnchor="middle" fill="#D9D2C4" fontSize="8" fontFamily="'JetBrains Mono', monospace">
            {p.name}
          </text>
        </g>
      ))}
...
async function uploadToStorage(fileOrBlob, filename) {
  const name = filename || fileOrBlob.name || "photo.jpg";
  const path = `${uid("img")}-${name}`.replace(/\s+/g, "_");
  ...
  if (!imgUrl || !natural.w) {
    return (
      <div style={styles.modalOverlay}>
        <div style={styles.modalCard}><p style={{ color: "#9AA39C" }}>Chargement de l'image…</p></div>
      </div>
    );
  }
...
function PhotoFormModal({ title, initial, pages, onClose, onSubmit }) {
  ...
  return (
    ...
    <Modal onClose={onClose}>
      ...
      <input type="file" accept="image/*" style={styles.input} onChange={onPickFile} disabled={uploading} />
...
function CountryFormModal({ onClose, onSubmit }) {
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <input style={styles.input} value={lat} onChange={(e) => setLat(e.target.value)} placeholder="Ex : 64" />
      <label style={styles.label}>Longitude</label>
      <input style={styles.input} value={lon} onChange={(e) => setLon(e.target.value)} placeholder="Ex : -19" />
      <button style={styles.primaryBtn} disabled={!name || lat === "" || lon === ""}
        onClick={() => onSubmit({ name, lat: parseFloat(lat), lon: parseFloat(lon) })}>
        Créer ce pays
      </button>
    </Modal>
  );
...
function extractMainRing(geojson) {
...
function decimateRing(ring, maxPoints = 400) {
...
async function fetchCountryBorder(name) {
  try {
    const url = `https://nominatim.openstreetmap.org/search?format=json&polygon_geojson=1&polygon_threshold=0.01&limit=1&featureType=country&q=${encodeURIComponent(name)}`;
    const res = await fetch(url);
    const results = await res.json();
    const ring = results && results[0] ? extractMainRing(results[0].geojson) : null;
    if (ring && ring.length > 0) return decimateRing(ring).map(([lon, lat]) => ({ lat, lon }));
  } catch (e) {
...
const styles = {
  ...
  indexList: { listStyle: "none", padding: 0, margin: 0 },
  indexItem: { borderBottom: "1px solid #2a2b27" },
  indexBtn: {
    width: "100%", background: "none", border: "none", color: "#F4EFE7", cursor: "pointer",
    display: "flex", justifyContent: "space-between", alignItems: "center",
    fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: "clamp(22px, 6vw, 30px)",
    textTransform: "uppercase", letterSpacing: 1, padding: "16px 4px",
  },
  indexCount: { fontFamily: FONT_MONO, fontSize: 13, color: "#B4794A", fontWeight: 400 },
  indexDeleteBtn: { background: "none", border: "none", color: "#9AA39C", fontSize: 16, padding: "8px 6px", cursor: "pointer" },
  ...
  countryMapSvg: { width: 260, height: 260 },
  editCoverBtn: {
  ...
  addPhotoFloating: { position: "absolute", bottom: 60, right: 16, fontFamily: FONT_BODY, fontSize: 13, color: "#0F1211", background: "#C9A876", border: "none", borderRadius: 10, padding: "9px 14px", cursor: "pointer", fontWeight: 600 },
