...
function playIntroSting() {
  try {
    ...
    const reverb = createSoftReverb(ctx, 3.4, 2.4);
    const bus = ctx.createGain();
    bus.connect(dry).connect(ctx.destination);
    bus.connect(wet).connect(reverb).connect(ctx.destination);

    // Montée en tension avant l'impact — un souffle filtré qui monte, façon "riser" de bande-annonce
    const riser = (start, dur) => {
      const noise = ctx.createBufferSource();
      noise.buffer = makeNoiseBuffer(ctx, dur);
      const filter = ctx.createBiquadFilter();
      filter.type = "bandpass";
      filter.frequency.setValueAtTime(180, start);
      filter.frequency.exponentialRampToValueAtTime(1600, start + dur);
      filter.Q.value = 0.9;
      const gain = ctx.createGain();
      gain.gain.setValueAtTime(0.0001, start);
      gain.gain.exponentialRampToValueAtTime(0.2, start + dur * 0.92);
      gain.gain.linearRampToValueAtTime(0.0001, start + dur + 0.05);
      noise.connect(filter).connect(gain).connect(bus);
      noise.start(start);
      noise.stop(start + dur + 0.1);

      const osc = ctx.createOscillator();
      osc.type = "sine";
      osc.frequency.setValueAtTime(58, start);
      osc.frequency.exponentialRampToValueAtTime(150, start + dur);
      const oGain = ctx.createGain();
      oGain.gain.setValueAtTime(0.0001, start);
      oGain.gain.exponentialRampToValueAtTime(0.16, start + dur * 0.9);
      oGain.gain.linearRampToValueAtTime(0.0001, start + dur + 0.05);
    ...
    const riserDuration = 1.15;
    riser(now, riserDuration);
    thump(now + riserDuration);
    bellHit(now + riserDuration + 0.5);

    setTimeout(() => ctx.close(), riserDuration * 1000 + 4200);
  } catch (e) {
